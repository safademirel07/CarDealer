...
$comments = getMaintenances($carID);
...
<body id="page-top">
          ...
          <div class="row">
            <div class="page-header">
              <div class="d-flex align-items-center">
                <h2 class="page-header-title">Maintenance of <?php echo getCarDetail($carID,"title")?></h2>
              </div>
            </div>
          </div>
          <!-- End Page Header -->
            ...
            <div class="col-xl-12">
              ...
              <div class="widget has-shadow">
                <div class="widget-body">
                  ...
                  <button class="add-modal" type="button" data-toggle="modal" data-id='<?php echo $carID ?>' data-date="<?php echo $datetime->format('Y-m-d\TH:i')?>"
                    ...
                    <div class="table-responsive">
                      <table class="table table-hover mb-0">
                        ...
                        <tbody>
                          ...
                          {
                            $phpdate = strtotime( $comment->last_time );
                            $mysqldate = date( 'Y-m-d H:i:s', $phpdate );
                            $datetimes = new DateTime($mysqldate);

                            echo '
                            <tr>
                            <td><span class="text-primary">' . $comment->id . '</span></td>
                            <td>' . $comment->description . '</td>
                            <td>' . $comment->last_time . '</td>
                            <td class="td-actions">
                            <button class="change-modal" data-toggle="modal" data-target="#change-modal" data-id=' . $comment->id . '  data-carid=' . $carID . ' data-name="' . $comment->description . '" data-date="' . $datetimes->format('Y-m-d\TH:i') . '" type="button"><i class="la la-edit edit"></i></button>
                            <button onclick="deleteF(' . $comment->id . ','.$carID.')"><i class="la la-close delete"></i></button>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="addMaintenanceForm">
              ...
              <label class="col-lg-3 form-control-label">Message</label>
              <div class="col-lg-9">
                <div class="input-group">
                  ...
                  <?php  $datetime = new DateTime($timeinout[0]->time_in);   ?>
                  <input type="datetime-local" value = "<?php echo $datetime->format('Y-m-d\TH:i'); ?>"  class="form-control" id="date" name="date">
                </div>
              </div>
              <br>
  ...
  <div id="change-modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Change Maintenance</h4>
          <button type="button" class="close" data-dismiss="modal">
            <span aria-hidden="true">×</span>
            <span class="sr-only">close</span>
          </button>
...
$(document).ready(function () {
  ...
  $(document).on("click", ".change-modal", function () {
    ...
    $(".col-lg-9 #id").val(id);
    $(".col-lg-9 #description").val(name);
    $(".col-lg-9 #date").val(date);
  });
...
function deleteF(id,carid) {
  bootbox.confirm("Do you want to delete maintenance", function (result) {
    if (result) {

      $.ajax({
        type: 'get',
        url: "inc/post/maintenance/deletemaintenance.php",
